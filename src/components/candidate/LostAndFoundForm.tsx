import {
  CandidateType,
  lostAndFoundSchema,
  newApplicationLostAndFoundSchema,
  NewApplicationLostAndFoundType,
...
export interface LostAndFoundFormProps {
  data: CandidateType;
  token: string;
}

export default function LostAndFoundForm(props: LostAndFoundFormProps) {
  ...
  const {
    ...
    resolver: zodResolver(newApplicationLostAndFoundSchema),
    mode: "onChange",
  });
  ...
  const onSubmit = async (data: NewApplicationLostAndFoundType) => {
    if (!image) {
      ...
      <Box sx={{ marginTop: "1rem" }}>
        ...
        <VStack spacing={6}>
          ...
          <FormControl>
            ...
            <Input placeholder="Name" {...register("employeeID")} />
          </FormControl>
